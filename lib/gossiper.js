...
var Gossiper = exports.Gossiper = function(port, seeds, address, opts) {
  ...
  //3 args specified, address can only be address or opts
  ...
  //2 or 3 args specified.
  //if 2, 'seeds' can be either seeds, address, or opts
  //if 3, same thing, but we need to allow for address to be specified.
  if(arguments.length >= 2 && argument.length < 4) {
    if(typeof seeds == typeof {} && !seeds.push) {
      opts = seeds;
  ...
  //todo?: arg value validation

  //set default opts if they aren't already set.
  opts = opts || {};
  opts.emitUpdateOnHeartBeat = opts.emitUpdateOnHeartBeat || false;
  opts.bind_address = opts.bind_address || address;

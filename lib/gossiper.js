var PeerState     = require('./peer_state').PeerState,
    ...
    dns           = require('dns'),
    debug         = require('debug')('chitchat_gossiper')
...
var Gossiper = exports.Gossiper = function(port, seeds, address, emitUpdateOnHeartBeat) {
  ...
  this.port     = port;
  this.seeds    = seeds || [];
  this.my_state = new PeerState(this.address, this.port);
...
Gossiper.prototype.start = function(callback) {
  ...
  if(this.address) {
    ...
    debug('%s started', this.peer_name);
...
Gossiper.prototype.gossip = function() {
  ...
  if(this.livePeers().length > 0) {
    live_peer = this.chooseRandom(this.livePeers());
...
Gossiper.prototype.gossipToPeer = function(peer) {
  ...
  gosipeeSocket.on('error', function(exception) {
    debug('%s error: %s' , self.peer_name, exception);
  });
}
...
Gossiper.prototype.handleNewPeers = function(new_peers) {
  ...
  for(var i in new_peers) {
    ...
    var tp = new PeerState(port, address);

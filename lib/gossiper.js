...
var Gossiper = exports.Gossiper = function(port, seeds, ip_to_bind, emitUpdateOnHeartBeat) {
  ...
  if(this.ip_to_bind) {
    ...
    child_process.exec('hostname', function(error, stdout, stderr) {
      ...
      dns.lookup(hostname, 4, function(err,address, family) {
        ...
        this.my_state.address = self.address;
        this.my_state.port = self.port;
...
Gossiper.prototype.gossipToPeer = function(peer) {
  ...
  var gosipeeSocket = new net.createConnection(a.port, a.address);
  var self = this;

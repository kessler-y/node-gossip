...
/*
  ...
  *	4. the initiating peer responds with a message otype SECOND_RESPONSE
  *
  *	@class
  */
...
Gossiper.prototype.gossipToPeer = function(peer) {
	...
	socket.on('connect', function() {
		socket.send(self.requestMessage());
	})
...
Gossiper.prototype.handleMessage = function(msg) {
	...
	switch (msg.type) {
		// the request message is always handled at the server peer
		case Gossiper.REQUEST:
			return this.firstResponseMessage(msg.digest);
		
		// the first response message is always handled by the client peer	
		...
		// the second response message is always handled at the server peer
